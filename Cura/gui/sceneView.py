...
class httpUploadDataStreamMKS(object):
    """
    ...
    def read(self, size):
        ...
        if self._progressCallback is not None:
            ...
            self._progressCallback(self._readPos, len(self._dataList))
        return ret
...
class SceneView(openglGui.glGuiPanel):
    def __init__(self, parent):
        ...
        self.screenshotReady = False
        self.isGcode = False
        self.uploadProcessDlg = None
        self.resp = None
        self.respisClose = False

    def loadGCodeFile(self, filename):
        print filename
        self.OnDeleteAll(None)
        #Cheat the engine results to load a GCode file into it.
        self._gcodeFileName = filename[filename.rfind("\\")+1:]
    ...
    def showSaveGCode(self):
        ...
        wildcardFilter = "GCode files (%s)|%s;%s" % (profile.getGCodeExtension(), profile.getGCodeExtension(), profile.getGCodeExtension()[0:2])
        wildcardFilter += "|TFT files (%s)|%s;%s" % (profile.getGCodeExtension(), profile.getGCodeExtension(), profile.getGCodeExtension()[0:2])
        dlg.SetWildcard(wildcardFilter)
        ...
        if index == 1:
            ...
            self.hiddenBtn()
            # self._selectObject(self._scene.objects()[0], True)
            # self._drawMachine()
            self.OnPaint(None, (0, 0, 0, 0))
            screenshot = self.take_screenshot()
            self.takescreenshot = False
            threading.Thread(target=self._saveGCodeTFT, args=(filename,screenshot,)).start()
        else:
    ...
    def take_screenshot(self):
        ...
        if size.GetWidth() > size.GetHeight():
            nw = int(math.ceil(size.GetHeight()))
            nx = int(math.ceil(size.GetWidth() - nw)/2)
        glPixelStorei(GL_UNPACK_ALIGNMENT, 4)
        # self.SwapBuffers()
        imagedata = None
        img = None
        for i in xrange(0, 3):
            imagedata = glReadPixels(nx, 0, nw, nw, GL_RGB, GL_UNSIGNED_BYTE)
            # print imagedata
            # img = Image.frombuffer("RGB", (nw, nw), imagedata)
            img = Image.frombuffer("RGB", (nw, nw), imagedata, 'raw', "RGB", 0, 1)
            # path = sys.path[0] + str(nx) + "_" + str(0) + "img.png"
    ...
    def progressCallback(self, readpos, totallen):
        ...
        if self.uploadProcessDlg is not None:
            if not self.uploadProcessDlg.Update(pro):
                print "self.uploadProcessDlg.Update(pro)"
                if self.uploadProcessDlg is not None:
                    self.uploadProcessDlg.Destroy()
                    self.uploadProcessDlg = None
                if self.resp is not None:
                    self.resp.close()
                self.respisClose = True

    def mkswifiStartPrint(self):
        print 'mkswifiStartPrint'
        gcode = None
        filename = None
        if len(self._scene.objects()) < 1 and self._engine._result is None:
            self.notification.message("Please load model first")
            return
        if len(self._scene.objects()) > 0:
            if not self._engine.getResult().isFinished():
                self.notification.message("Please wait before loading model")
                return
            ...
            self.isGcode = True
            gcode = self._engine._result.getGCode()
            if self._gcodeFileName is None:
                filename = "cura_mkswifi.gcode"
            else:
                filename = self._gcodeFileName
        filename = self.validateFilename(filename)
        if not filename:
            self.notification.message("Canceled send job")
        ...
        if not self.isGcode:
            ...
            # self.screenshotReady = False
            self.takescreenshot = True
            # self._engineResultView.setEnabled(False)
            ...
            screenshot = self.take_screenshot()
            self.takescreenshot = False
            # print "take_screenshot ok---"
            single_string_file_data += self.add_screenshot(screenshot, 100, 100, ";simage:")
            single_string_file_data += self.add_screenshot(screenshot, 200, 200, ";;gimage:")
            # print "add_screenshot ok---"
        self.takescreenshot = False
        for line in gcode:
        ...
        # data.append(single_string_file_data.encode(sys.getfilesystemencoding()))
        # data.append('--%s--\r\n' % boundary)
        ...
        for i in xrange(retrymax):
            try:
                ...
                if self.uploadProcessDlg is not None:
                    self.uploadProcessDlg.Destroy()
                    self.uploadProcessDlg = None
                # self.uploadProcessDlg = wx.ProgressDialog("File is uploading, please wait.", "uploading", style=wx.PD_APP_MODAL | wx.PD_CAN_ABORT | wx.PD_ESTIMATED_TIME | wx.PD_REMAINING_TIME)
                ...
                if self.resp is not None:
                    self.resp = None
                self.resp = urllib2.urlopen(req, timeout=500)
                if self.respisClose:
                    print "close"
                    if self.uploadProcessDlg is not None:
                        self.uploadProcessDlg.Destroy()
                        self.uploadProcessDlg = None
                    self.notification.message("Upload has been cancel")
                    return {"status": False, "filename": ""}
                    break
                # get response
                qrcont = self.resp.read()
                # senddata = req.get_data()
                print("qrcont===" + qrcont)
                print("ok=========================")
                if self.uploadProcessDlg is not None:
                    self.uploadProcessDlg.Destroy()
                    self.uploadProcessDlg = None
                if qrcont == "{\"err\":0}":
                    self.notification.message("Upload success")
    ...
    def hiddenBtn(self):
        self.rotateToolButton.setHidden(True)
        self.scaleToolButton.setHidden(True)
        self.mirrorToolButton.setHidden(True)
        self.resetRotationButton.setHidden(True)
        self.layFlatButton.setHidden(True)
    ...
    def validateFilename(self, filename):
        ...
        coverfilename = False
        wifipanel = self.GetParent().GetParent().GetParent().normalSettingsPanel.mkswifiPanel
        inlist = wifipanel.getNameInList(filename)
        while (inlist and not coverfilename) or len(filename[filename.rfind("\\")+1:]) >= 30:
            if inlist and not coverfilename:
                mydlg = wx.TextEntryDialog(self, _("The file %s already exists.Do you want to rename it?") % filename, _("Tips"), filename)
                if mydlg.ShowModal() == wx.ID_OK:
                    filename = mydlg.GetValue()
                    inlist = wifipanel.getNameInList(filename)
                else:
                    coverfilename = True
                    break
            ...
            if len(filename[filename.rfind("\\")+1:]) >= 30:
                mydlg = wx.TextEntryDialog(self, _("File name %s is too long to upload, please rename it") % filename, _("Tips"), filename)
                if mydlg.ShowModal() == wx.ID_OK:
                    filename = mydlg.GetValue()
                else:
                    return False
    ...
    def _saveGCodeTFT(self, targetFilename, screenshot, ejectDrive = False):
        gcode = self._engine.getResult().getGCode()
        try:
            size = float(len(gcode))
            read_pos = 0
            with open(targetFilename, 'wb') as fdst:
                fdst.write(self.add_screenshot(screenshot, 100, 100, ";simage:"))
                fdst.write(self.add_screenshot(screenshot, 200, 200, ";;gimage:"))
                while 1:
            ...
            if ejectDrive:
                self.notification.message("Saved as TFT %s" % (targetFilename), lambda : self._doEjectSD(ejectDrive), 31, 'Eject')
            elif explorer.hasExplorer():
                self.notification.message("Saved as TFT %s" % (targetFilename), lambda : explorer.openExplorer(targetFilename), 4, 'Open folder')
            else:
                self.notification.message("Saved as TFT %s" % (targetFilename))
    ...
    def getMouseRay(self, x, y):
        ...
        # print self._modelMatrix
        if self._viewport is None:
            return numpy.array([0,0,0],numpy.float32), numpy.array([0,0,1],numpy.float32)
        p0 = numpy.array([0,0,0],numpy.float32)
        p1 = numpy.array([0,0,0],numpy.float32)
        try:
            ...
            print "getMouseRay---------------Exception"
            print e
        return p0, p1

    def _init3DView(self, color=None):
    ...
    def OnPaint(self,e, color=None):
        ...
        if self._mouseX > -1: # mouse has not passed over the opengl window.
            ...
            try:
                ...
                print "onpaint-self._mouse3Dpos---------------Exception"
                print e

        self._init3DView(color)
        if self.takescreenshot:
            glTranslate(0, 0, -numpy.max(self._machineSize) * 2.5)
            # glTranslate(0, 0, -self._zoom)
            glRotate(-80, 1, 0, 0)
            glRotate(20, 0, 0, 1)
            glTranslate(-self._viewTarget[0], -self._viewTarget[1], -self._viewTarget[2])
        ...
        if self.viewMode != 'gcode':
            ...
            if self._objectLoadShader is not None:
                ...
                glDisable(GL_BLEND)
        # print "ready to _drawMachine"
        if not self.takescreenshot:
            self._drawMachine()
        ...
        # print "end paint"
        if self.takescreenshot:
            self.screenshotReady = True
        # if self.takescreenshot and self.screenshotReady:
            # threading.Thread(None, self.take_screenshot(), "takescreeshot")
