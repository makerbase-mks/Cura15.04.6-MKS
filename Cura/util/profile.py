...
#mkswifi
setting('ip_address', '', str, 'machine', 'hidden').setLabel(_("IP Address"), _("The IP Address of your machine."))
setting('extrCurTemp', '', int, 'machine', 'hidden').setLabel(_("Current Temp"), _("The extruder current temperature of your machine."))
setting('printer_file_list', '1', ['1'], 'machine', 'hidden')
...
def getBasePath():
    ...
    if platform.system() == "Windows":
        ...
        basePath = os.path.abspath('') + '\.config'
    elif platform.system() == "Darwin":
        # basePath = os.path.expanduser('~/Library/Application Support/Cura/%s' % version.getVersion(False))
        basePath = os.path.expanduser('~/Library/Application Support/Cura-MKS/%s' % version.getVersion(False))
    else:
